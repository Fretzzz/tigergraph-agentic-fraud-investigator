...
export function investigate(slice: CaseSlice, opts: { runId: string; startedAtMs?: number; nowMs?: () => number; queries?: InvestigationQueries }): InvestigationRun {
  ...
  step("Does this connect to other customers or cards?", sr, `${sr.result.otherCustomerTxnsSharingRegionAndEmail.length} other-customer transactions ${seed.region ? `share region ${seed.region}` : "also have no billing region"} and share email domain ${seed.purchaser_email_domain} in the ${slice.scope.neighborhood_hours}h window (${slice.region_window.transactions} region transactions from ${slice.region_window.other_customers} other customers overall); other-customer closed fraud cases linked: ${sr.result.otherCustomerFraudCases.length}.`);
  ...
  const evidence: Answer["case"]["evidence"] = [
    ...
    ...(hr.result.length ? [{ claim: `${hr.result.length} prior closed cases on ${c.card_id}; confirmed patterns: ${confirmedPatterns.join(", ") || "none"}. ${priorFraudRegions.has(seed.region ?? "") ? `A prior confirmed fraud transaction was billed in region ${seed.region ?? "(none recorded)"}.` : `No prior confirmed fraud transaction was billed in region ${seed.region ?? "(none recorded)"} (prior fraud regions: ${[...priorFraudRegions].join(", ") || "none"}).`}`, source: "graph" as const, ref: hr.receiptId, entity_ids: hr.result.map(h => h.id) }] : []),
    { claim: `No other customer shares ${seed.region ? `region ${seed.region}` : "the missing billing region"} and email domain ${seed.purchaser_email_domain} in the ${slice.scope.neighborhood_hours}h window, and no other customer's fraud case links to this card.`, source: "graph" as const, ref: sr.receiptId, entity_ids: [seed.id] },
