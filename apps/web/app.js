...
async function main() {
  ...
  ${tg ? `<section class="panel full"><h2>TigerGraph run</h2>
    ...
    <p class="muted">Not in TigerGraph (kept local): ${d.tigergraph.fieldsNotInTigerGraph.map(esc).join(", ")}. ${d.tigergraphCaseWrite ? "The case record is written back to TigerGraph (see below)." : "Case record write-back stays local."}</p></section>` : ""}
  <section class="panel"><h2>Limitations of this run</h2><ul class="lim">${d.limitations.map(l => `<li>${esc(l)}</li>`).join("")}</ul>
    ${d.tigergraphCaseWrite ? `<p class="muted">Case written to TigerGraph Savanna: ${esc(d.tigergraphCaseWrite.vertexType)} <b>${esc(d.tigergraphCaseWrite.vertexId)}</b> on graph ${esc(d.tigergraphCaseWrite.graph)}, readback ${d.tigergraphCaseWrite.readbackVerified ? "verified" : "FAILED"} (${esc(d.tigergraphCaseWrite.writtenAt)} UTC)${d.tigergraphCaseWrite.edges && d.tigergraphCaseWrite.edges.length ? `; linked by ${d.tigergraphCaseWrite.edges.map(esc).join(", ")}` : "; no edges, because this case's transaction and card are not loaded in TigerGraph"}. written_to_graph=true.</p>` : ""}
    <p class="muted">Local case write ${esc(d.localCaseWrite.vertexId)}: readback ${d.localCaseWrite.readbackVerified ? "verified" : "FAILED"} (local graph copy).</p></section>`;
